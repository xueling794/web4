package com.qixi.common.spring;
...
public class AnnotationMethodHandlerAdapter extends WebContentGenerator implements HandlerAdapter, Ordered,
    ...
    public AnnotationMethodHandlerAdapter() {
        ...
        this.messageConverters = new HttpMessageConverter[] { new ByteArrayHttpMessageConverter(),
                stringHttpMessageConverter, new SourceHttpMessageConverter(), new XmlAwareFormHttpMessageConverter() };
    }
    ...
    private void writeNoLoginResponse(HttpServletResponse response, Map<String, Object> resultMap) throws Exception {
        ...
        try {
            ...
            resultMap.put("resultMessage", "登陆已过期");
            resultMap.put("contextCode", 0);
            resultMap.put("responseMessage", "登陆已过期");
    ...
    protected ModelAndView invokeHandlerMethod(HttpServletRequest request, HttpServletResponse response, Object handler)
        ...
        // modify by ling 2015-01-15 begin
        String ajaxHeaderKey = "X-Requested-With";
        String ajaxHeaderValue = "XMLHttpRequest";
        boolean needAnth = true;

        Method handlerMethodToInvoke = BridgeMethodResolver.findBridgedMethod(handlerMethod);
        if (handlerMethodToInvoke.isAnnotationPresent(RequestAuthentication.class)) {
            RequestAuthentication requestAuthentication = handlerMethodToInvoke
                    .getAnnotation(RequestAuthentication.class);
            needAnth = requestAuthentication.isAuth();

        }
        // need anth
        if (needAnth) {
            String uri = request.getRequestURI();
            if (uri != null && (uri.indexOf("restful") != -1 || uri.endsWith(".do") || uri.endsWith(".htm"))) {
                Object userBase = request.getSession().getAttribute(UserConst.USER_BASE);
                if (userBase == null) {
                    String ajaxHeaderValueParam = request.getHeader(ajaxHeaderKey);
                    HttpServletResponse httpResponse = (HttpServletResponse) response;
                    if (ajaxHeaderValueParam != null && ajaxHeaderValueParam.equals(ajaxHeaderValue)) {
                        // ajax request
                        this.writeNoLoginResponse(httpResponse, null);
                    } else {
                        // 非ajax request 跳转页面
                        httpResponse.sendRedirect("/login.html?preUrl=" + request.getRequestURL());
                    }
                    return null;
                }
            }
        ...
        // modify by yusong /2013-08-29 end
    ...
    /**
     * This method always returns -1 since an annotated controller can have many
     * methods, each requiring separate lastModified calculations. Instead, an
     *
     * @{@link RequestMapping}-annotated method can calculate the lastModified
     *         value, call
